<div class="text-center animate-box" data-animate-effect="fadeInLeft">
    <hr>
    <div class="row">
        <div class="col-sm-12">
            <button class="btn btn-light btn-block btn-sm" data-clipboard-text="{{ Request::url() }}">
                <i class="fas fa-link"></i> คัดลอกลิ้งค์
            </button>
        </div>
        <div class="col-sm-6">
            <a class="btn btn-success btn-block btn-sm" href="https://line.me/ti/p/~ratasak1234"
                target="_blank">สั่งซื้อคลิก (แอดไลน์ ratasak1234)</a>
        </div>
        <div class="col-sm-6">
            <a class="btn btn-block btn-danger btn-sm"
                href="{{ url('page/view/23') }}">แอดไลน์ร้านไม่ได้คลิกที่นี่จ้า</a>
        </div>
    </div>
    <hr>
</div>